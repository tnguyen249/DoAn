function nImageTrue  = hogSvm(m,n)
    lblTest = loadMNISTLabels('t10k-labels.idx1-ubyte');
    if exist(['HOGSVM',num2str(m),num2str(n),'.mat'], 'file') == 2
        load(['HOGSVM',num2str(m),num2str(n),'.mat']);
    else
        ...
        Mdl = fitcecoc(imgTrain_hist',lblTrain);
        ...
        save(['HOGSVM',num2str(m),num2str(n),'.mat'],'lblResult');
    end
